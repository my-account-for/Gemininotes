...
def render_output_and_history_tab(state: AppState):
    ...
    # Get the summary list for the sidebar/history
    notes = database.get_all_notes()
    
    if not notes:
        st.info("No notes generated. Go to the 'Input & Generate' tab to create one.")
    else:
        # Ensure we have a valid active ID
        if not state.active_note_id or not any(n['id'] == state.active_note_id for n in notes):
            state.active_note_id = notes[0]['id']

        # --- FIX: FETCH FULL DATA INCLUDING TRANSCRIPTS ---
        # Instead of taking the incomplete object from 'notes', we query the DB for the full record.
        active_note = database.get_note_by_id(state.active_note_id)
        
        # Fallback in case the specific ID fetch fails for some reason
        if not active_note:
