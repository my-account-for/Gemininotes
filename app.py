...
# REMOVED: from streamlit_copy_to_clipboard import st_copy_to_clipboard
...
# (Keep state initialization as is)
default_state = {
...
def update_prompt_display_text():
    ...
    if st.session_state.view_edit_prompt_enabled and meeting_type != "Custom":
        ...
        if input_type == "Upload Audio":
             ...
             st.session_state.current_prompt_text = ("# NOTE FOR AUDIO INPUT: ...\n#######\n\n" + base_prompt)
...
# --- Input Section (Keep structure as is) ---
with st.container(border=True):
    ...
    with col_main_2: st.subheader(""); st.button("🧹 Clear All", on_click=clear_all_state, use_container_width=True, help="Reset all inputs and outputs.")
    st.divider()
...
with output_container:
    ...
    if st.session_state.processing: st.info("⏳ Processing...", icon="🧠")
        ...
        # --- Action Buttons (Copy removed) ---
        st.write("")
        col_btn_1, col_btn_2 = st.columns(2) # Adjust layout now
        with col_btn_1: # TXT Download
             st.download_button(label="⬇️ TXT", data=notes_content_to_use, file_name=f"{st.session_state.selected_meeting_type.lower().replace(' ', '_')}_notes.txt", mime="text/plain", key='download-txt', help="Download as Plain Text")
        with col_btn_2: # MD Download
             st.download_button(label="⬇️ Markdown", data=notes_content_to_use, file_name=f"{st.session_state.selected_meeting_type.lower().replace(' ', '_')}_notes.md", mime="text/markdown", key='download-md', help="Download as Markdown File")

         # Optional: Keep DOCX fallback if needed and python-docx is installed
         # try:
         #     docx_bytes = create_docx(notes_content_to_use)
         #     st.download_button(label="⬇️ DOCX", data=docx_bytes, file_name=f"{st.session_state.selected_meeting_type.lower().replace(' ', '_')}_notes.docx", mime="app...", key='download-docx')
...
# --- Processing Logic (Keep try...finally structure) ---
if generate_button:
...
if st.session_state.processing:
    try: # Outer try for overall processing flow
        # Retrieve state
        meeting_type = st.session_state.selected_meeting_type
        ...
        user_prompt_text = st.session_state.current_prompt_text # Get latest from state
        ...
        if not st.session_state.error_message:
            if prompt_was_edited_or_custom:
                ...
                if actual_input_type != "Upload Audio":
                    ...
                    final_prompt_for_api = ("1. First, transcribe...\n2. Then, create notes based on:\n---\n" + base_prompt)
        ...
        if not st.session_state.error_message and final_prompt_for_api and (transcript_data or audio_file_obj):
            ...
            try:
                ...
                # Cleanup Audio AFTER successful call
