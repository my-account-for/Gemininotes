...
import PyPDF2
import docx # Still needed for DOCX fallback if TXT fails
...
<style>
    ...
    .stButton > button:disabled, .stButton > button[kind="primary"]:disabled { background-color: #D1D5DB; color: #6B7280; box-shadow: none; transform: none; cursor: not-allowed; }
     /* Secondary Button styling for Clear/Copy */
    ...
    .stButton>button.secondary-button:hover { background-color: #E5E7EB; border-color: #9CA3AF; }
     /* Download Buttons */
    .stDownloadButton > button { border-radius: 0.5rem; padding: 0.6rem 1.2rem; font-weight: 500; background-color: #F3F4F6; color: #1F2937; border: 1px solid #D1D5DB; transition: background-color 0.2s ease-in-out; width: auto; margin-top: 0; } /* Removed margin-top */
...
def get_current_input_data():
    ...
    if input_type == "Paste Text": transcript = st.session_state.text_input.strip()
        ...
        # Get file content only if a file is actually uploaded
...
def clear_all_state():
    ...
    st.session_state.pdf_uploader = None # Clears the reference
    st.session_state.audio_uploader = None # Clears the reference
    ...
    # Optionally reset selections (might be slightly annoying for user)
    ...
    # Manually trigger update for prompt area after clearing state
    update_prompt_display_text()
    st.toast("Inputs and outputs cleared!", icon="🧹")
    # Note: To visually clear file uploaders, using st.rerun() after setting keys to None
    # might be needed, or managing them via their own widget keys if direct state clear isn't enough.
...
with st.container(border=True):
    ...
    input_type_ui = st.session_state.input_method_radio # Use state for conditional render
    if input_type_ui == "Paste Text": st.text_area("Paste transcript:", height=150, key="text_input", placeholder="Paste the full meeting transcript here...\nExample:\nSpeaker A: Let's start the meeting.\nSpeaker B: Okay, the main topic is...")
...
if show_prompt_area:
    with st.container(border=True):
        ...
        # Bind text_area directly to current_prompt_text state
        st.text_area(
            label="Prompt Text:", value=st.session_state.current_prompt_text, key="current_prompt_text",
...
with output_container:
    ...
    if st.session_state.processing: st.info("⏳ Processing...", icon="🧠")
        ...
        # Edit Notes Checkbox
        st.checkbox("Edit Notes", key="edit_notes_enabled", help="Enable editing the generated notes before download/copy.")
        if st.session_state.edit_notes_enabled:
             # Bind text area directly to edited_notes_text state
             st.text_area("Editable Notes:", value=st.session_state.edited_notes_text, key="edited_notes_text",
        ...
        with col_btn_1: st_copy_to_clipboard(notes_content_to_use, label="📋 Copy", success_message="Notes copied!", key="copy_notes")
        with col_btn_2: st.download_button(label="⬇️ TXT", data=notes_content_to_use, file_name=f"{st.session_state.selected_meeting_type.lower().replace(' ', '_')}_notes.txt", mime="text/plain", key='download-txt', help="Download as Plain Text")
        with col_btn_3: st.download_button(label="⬇️ Markdown", data=notes_content_to_use, file_name=f"{st.session_state.selected_meeting_type.lower().replace(' ', '_')}_notes.md", mime="text/markdown", key='download-md', help="Download as Markdown File")
        # Optional DOCX fallback can be added here if needed
    else: st.markdown("<p class='initial-prompt'>Generated notes will appear here.</p>", unsafe_allow_html=True)


# --- Processing Logic ---
if generate_button:
    # Reset outputs, keep inputs/selections, set processing flag
...
if st.session_state.processing:
    ...
    try:
        ...
        # Get latest prompt text directly from state (updated by text_area)
        ...
        if not st.session_state.error_message and final_prompt_for_api and (transcript_data or audio_file_obj):
            ...
            try:
                ...
                # Cleanup Audio AFTER successful call if audio was used
                ...
                # Catch specific API/processing errors and store message
                st.session_state.error_message = f"❌ Processing Error: {e}"
                # Attempt audio cleanup on this inner error too
                if st.session_state.uploaded_audio_info:
                    try: genai.delete_file(st.session_state.uploaded_audio_info.name); st.session_state.uploaded_audio_info = None
                    except Exception: pass
        # End of inner try...except for API/processing

    # --- FINALLY block: ALWAYS runs after the outer try ---
    finally:
        st.session_state.processing = False
        # Rerun ONLY if there was an error OR if generation was successful
        # This prevents unnecessary reruns if validation failed early
        if st.session_state.error_message or st.session_state.generated_notes:
            st.rerun()
        # If validation failed, the error is already set, and the UI will update
        # on the next interaction without needing an immediate rerun here.

# --- Footer ---
st.divider()
st.caption("Powered by [Google Gemini](https://deepmind.google.com/technologies/gemini/) | App by SynthNotes AI")
