...
from prompts import (
    ...
    INTERNAL_DISCUSSION_SEGMENT_PROMPT,
...
# Internal Discussion notes are generated by decomposing the transcript into
# topics and writing each topic in its own short, focused call (see
# _generate_internal_discussion_decomposed). This stops the single long
# generation from drifting back into the raw conversation's Q&A shape near the
# end. Below this word count a transcript is short enough that a single pass
# does not drift, so the extra segmentation call is not worth it.
MIN_DECOMPOSE_WORDS = 500
...
def _additional_context_block(state: AppState) -> str:
    """The ADDITIONAL CONTEXT block (typed + transcribed voice note), or "".

    Mirrors the context assembly in get_dynamic_prompt so the decomposed
    Internal Discussion path carries the same user context into every
    per-topic call."""
    ...
    if state.add_context_enabled and combined:
        return f"**ADDITIONAL CONTEXT:**\n{combined}"
    return ""


def _segment_internal_discussion(transcript: str, notes_model) -> Tuple[List[Dict[str, str]], int]:
    """Phase A of the decomposed Internal Discussion path: one cheap pass that
    returns an ordered list of ``{"title", "scope"}`` topics. Returns
    ``(topics, tokens)``; an empty list signals the caller to fall back to a
    single-pass generation. Parsing tolerates markdown code fences and stray
    prose the same way _discover_topics does."""
    prompt = INTERNAL_DISCUSSION_SEGMENT_PROMPT.format(transcript=transcript)
    response = generate_with_retry(notes_model, prompt, generation_config=GENERATION_CONFIG)
    tokens = safe_get_token_count(response)
    ...
    for t in raw_topics:
        ...
        if not title:
            continue
        topics.append({"title": title, "scope": str(t.get("scope", "")).strip()})
    return topics, tokens


def _generate_internal_discussion_decomposed(
    state: AppState, final_transcript: str, notes_model, progress, speaker_legend: str,
) -> Optional[Tuple[str, int]]:
    """Decomposed Internal Discussion generation: segment the transcript into
    topics (Phase A), then write each topic in its own short, focused call
    (Phase B, in parallel). Keeping each generation short and single-purpose
    stops the model from drifting back into the raw conversation's Q&A shape
    near the end of a long single pass.

    Returns ``(notes, tokens)``, or ``None`` to signal the caller to fall back
    to the single-pass path (segmentation failed or found fewer than two
    distinct topics, so decomposition would add cost without benefit)."""
    words = final_transcript.split()
    progress.update("generate", 0.03, f"{len(words):,} words · identifying topics")
    try:
        topics, seg_tokens = _segment_internal_discussion(final_transcript, notes_model)
    except Exception:
        return None
    # Fewer than two topics: nothing to decompose — let the single pass handle
    # it (one focused topic is just the single-pass case with extra overhead).
    if len(topics) < 2:
        return None

    n = len(topics)
    base_prompt = _speaker_legend_block(speaker_legend) + INTERNAL_DISCUSSION_PROMPT
    context_block = _additional_context_block(state)
    transcript_block = f"**MEETING TRANSCRIPT:**\n{final_transcript}"

    def _build_prompt(idx: int, topic: Dict[str, str]) -> str:
        focus = INTERNAL_DISCUSSION_TOPIC_FOCUS.format(
            index=idx + 1, title=topic["title"], scope=topic.get("scope", ""),
        )
        parts = [base_prompt, focus]
        if context_block:
    ...
    def _write_topic(idx: int):
        prompt = _build_prompt(idx, topics[idx])
        try:
            response = generate_with_retry(notes_model, prompt, generation_config=GENERATION_CONFIG)
            text = _extract_response_text(response)
            ...
            text, finish, tokens = "", "", 0
        # Internal Discussion legitimately condenses, so there is no word
        # floor; retry only rescues an empty or early-stopped topic.
        text, finish, extra = _retry_incomplete_notes(
            notes_model, prompt, text, finish, 0, progress, f"Topic {idx + 1} of {n}",
        )
        return idx, text, tokens + extra

    done_count = 0
    with ThreadPoolExecutor(max_workers=min(3, n)) as executor:
        futures = {executor.submit(_write_topic, i): i for i in range(n)}
        for future in as_completed(futures):
            idx, text, tokens = future.result()
            results[idx] = text
            topic_tokens[idx] = tokens
            done_count += 1
            progress.update(
                "generate",
                min(0.05 + 0.9 * done_count / n, 0.95),
                f"{done_count}/{n} topics written",
            )

    written = [r.strip() for r in results if r and r.strip()]
    if not written:
        # Every topic came back empty — fall back rather than return nothing.
        return None
    # Topics restart numbering only in the model's head; renumber deterministically
    # so the assembled document reads 1, 2, 3, … regardless of what each call emitted.
    notes = renumber_topic_sections("\n\n".join(written))
    return notes, seg_tokens + sum(topic_tokens)
...
def generate_notes_from_transcript(
    ...
    *,
    ...
    if skip_chunking or len(words) <= chunk_size:
        # Internal Discussion drifts back into a Q&A shape near the end of a
        # long single pass. Decompose it — find the topics, then write each in
        # its own short, focused call — instead of writing the whole document
        # in one go. Falls through to the single pass below when segmentation
        # fails or finds too few topics to be worth the extra call.
        if (
            state.selected_meeting_type == "Internal Discussion"
            and not skip_chunking
            and len(words) >= MIN_DECOMPOSE_WORDS
        ):
            decomposed = _generate_internal_discussion_decomposed(
                state, final_transcript, notes_model, progress, speaker_legend,
            )
