...
-   **ATTRIBUTION FREQUENCY:** Put the bolded name ONLY on the first bullet of a speaker's contribution, and again when the speaker changes mid-answer. Bullets that continue the same speaker's answer are plain `- ` bullets with no name prefix — do NOT repeat the same name on every bullet.
...
2.  **MID-ANSWER STARTS:** If the new section begins in the middle of a response, capture that content as bullet points under a clear question heading inferred from the context block, and end that heading with the marker `(contd.)` inside the bold (e.g., `**How does the pricing model work? (contd.)**`). The marker lets the stitching step attach these bullets to the same question in the previous section's notes. Use it ONLY on this first, mid-answer heading — never on any other heading. Do not skip or discard partial content.
